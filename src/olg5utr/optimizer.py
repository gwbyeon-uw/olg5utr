...
from olg5utr.encoding import DNA_ALPHABET, AA_ALPHABET, dna_to_onehot
from olg5utr.models import Translator, Optimus, OptimusOLG
...
class GradientOptimizer:
    ...
    def _clip_grad_per_batch(
        ...
        max_grad_norm = (factor * grad_norm).view(-1, 1, 1)
...
class SimulatedAnnealer:
    ...
    def _anneal(
        ...
        - Accept deteriorations with probability exp(-delta_E/T) where:
          * delta_E = loss_perturbed - loss (energy increase)
          * T = tau (temperature)
        ...
        Args:
            ...
            tau: Temperature parameter controlling acceptance probability.
                 ...
                 - tau -> inf: Accept all moves (random walk)
    ...
    def mutate_inplace(
        ...
        if target_positions is None:
            # Use unique positions per batch to avoid double-writes
            num_mutations = min(num_mutations, L)
            positions = torch.stack([torch.randperm(L, device=device)[:num_mutations] for _ in range(B)])
        else:
            positions = target_positions
        batch_idx = torch.arange(B, device=device).unsqueeze(1)
        current_nucs = torch.argmax(sequences[batch_idx, :, positions], dim=-1) # Get current and compute new nucleotides; gathered shape is (B, num_mutations, C)
...
class SynonymousMutator:
    ...
    def _build_lookup_tables(self, codon_table: Optional[dict] = None):
        ...
        max_syns = np.unique([ v for v in standard_dna_table.forward_table.values() ], return_counts=True)[1].max().item() - 1
...
class OptimusOLGPipeline:
    ...
    def __init__(self, config: OptimizationConfig = None):
        ...
        if isinstance(self.config.start_codon, str):
            self.start_codon = dna_to_onehot(self.config.start_codon, 3).transpose(1, 0).unsqueeze(0).repeat(self.config.n_batch, 1, 1)
        else:
